...
void SPECIE::position_parallel_pbc()
{
  ...
  for (int direction = 0; direction < mygrid->getDimensionality(); direction++)
  {
...
#endif
#else
      if (val[direction][p] > mygrid->rmaxloc[direction])
      {
        nlost++;
        nright++;
        if (mygrid->rmyid[direction] == mygrid->rnproc[direction] - 1)
          val[direction][p] -= Length;
        sendr_buffer = (double*)realloc(sendr_buffer, nright*Ncomp*sizeof(double));
        for (c = 0; c < Ncomp; c++)
          sendr_buffer[c + Ncomp*(nright - 1)] = val[c][p];

      }
      else if (val[direction][p] < mygrid->rminloc[direction])
      {
        nlost++;
        nleft++;
        if (mygrid->rmyid[direction] == 0)
        ...
        sendl_buffer = (double*)realloc(sendl_buffer, nleft*Ncomp*sizeof(double));
        for (c = 0; c < Ncomp; c++)
          sendl_buffer[c + Ncomp*(nleft - 1)] = val[c][p];
      }
      else
      {
        for (c = 0; c < Ncomp; c++)
          val[c][(p - nlost)] = val[c][p];
      }
#endif
    }
    ...
    for (int pp = 0; pp < nnew; pp++) {
      for (c = 0; c < Ncomp; c++) {
        val[c][(pp + nold - nlost)] = recv_buffer[pp*Ncomp + c];
      }
    }
#endif
  }
...
void SPECIE::momenta_advance(EM_FIELD *ebfield)
{
  ...
  const int edge = mygrid->getEdge();
  const int ebComp = ebfield->getNcomp();
  int N_grid[3];
  ebfield->writeN_grid(N_grid);
  const int Nx = N_grid[0];
  const int Ny = N_grid[1];
  const int Nz = N_grid[2];
  switch (mygrid->getDimensionality())
  {
    ...
    case 1:
      ...
      for (int p = 0; p < Np; p++){
        ...
        double hiw[3][3], wiw[3][3];  // half integer weight,  whole integer weight
        double rr, rh, rr2, rh2;          // local coordinate to integer grid point and to half integer,     local coordinate squared
        double xx[3];           // tensor_product,       absolute particle position
        ...
        rr = mygrid->dri[0] * (xx[0] - mygrid->rminloc[0]);
        rh = rr - 0.5;
        wii[0] = (int)floor(rr + 0.5); //whole integer int
        hii[0] = (int)floor(rr);     //half integer int
        rr -= wii[0];
        rh -= hii[0];
        rr2 = rr*rr;
        rh2 = rh*rh;
        ...
        wiw[0][1] = 0.75 - rr2;
        wiw[0][2] = 0.5*(0.25 + rr2 + rr);
        wiw[0][0] = 1. - wiw[0][1] - wiw[0][2];

        hiw[0][1] = 0.75 - rh2;
        hiw[0][2] = 0.5*(0.25 + rh2 + rh);
        hiw[0][0] = 1. - hiw[0][1] - hiw[0][2];
        ...
        const int k1 = 0, k2 = 0, j1 = 0, j2 = 0;
        //cilk::reducer_opadd<double> E[3], B[3];
        for (int i = 0; i < 3; i++)
        {
          i1 = i + wii[0] - 1;
          i2 = i + hii[0] - 1;
